...
class Convlution:
    def __init__(self, tensor_grap, node_weights, node_inputs, node_attribute, *args, **kwargs) -> None:
        ...
        if len(node_inputs) == 3:
            if node_inputs[2] in node_weights:
                bias = node_weights[node_inputs[2]]
            else:
                bias = tensor_grap[node_inputs[2]]
        else:
            bias = None
        if group == 1:
            ...
            if USE_NATIVE_GROUP_CONV:
                self.conv = TFConv(
                    in_channel, out_channel, kernel_shape, strides, dilations, pads, weights, bias, group=group
                )
                LOG.warning(
                    f"Group Convolution is detected, using native method, only supported tflite version >= 2.9, \
                                if compatibility error occurs and please make USE_NATIVE_GROUP_CONV=False!"
                )
            else:
                self.conv = TFGroupConv(
                    in_channel, out_channel, kernel_shape, strides, dilations, pads, group, weights, bias
                )
            ...
            if pads is not None and max(pads) == 1 and max(strides) == 1:
                self.convs.append(
                    keras.layers.Conv2D(
                        ...
                        use_bias=False if bias is None else True,
                        dilation_rate=dilations,
                        weights=[weights[:, :, :, i * out_channel_num : (i + 1) * out_channel_num]]
                        if bias is None
                        else [
                            weights[:, :, :, i * out_channel_num : (i + 1) * out_channel_num],
